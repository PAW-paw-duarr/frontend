...
interface ProfileOrangBaseProps {
  profileId: string;
  onClose: () => void;
}

export function ProfileOrangBase({ profileId, onClose }: ProfileOrangBaseProps) {
  ...
  return (
    <Dialog open={stateProfile} onOpenChange={handleOpenChange}>
      <DialogContent className="max-h-[80vh] overflow-y-auto bg-white sm:max-w-[400px]">
        <DialogHeader>
            ...
            <div className="space-y-4">
              ...
              {accountData?.cv_url ? (
                ...
                <Field>
                  ...
                  <div className="mb-3 flex items-center gap-2 rounded-lg border border-gray-200 bg-gray-50 p-3">
                    <FileText className="h-5 w-5 text-gray-400" />
                    <div className="flex-1">
                      <p className="text-sm font-medium text-gray-600">No CV uploaded</p>
                      <p className="text-xs text-gray-500">This user has not uploaded a CV yet</p>
                    </div>
                  </div>
                </Field>
              )}
