...
interface SidebarTitleProps {
  opens: boolean;
  setOpens: (open: boolean) => void;
}
export function SidebarTitle({ opens, setOpens }: SidebarTitleProps) {
  const uploadFile = useUploadFile();

  const [hash] = useHash();
  const navigate = useNavigate();
  const titleId = hash.startsWith('#title/') ? hash.replace('#title/', '') : '';
  ...
  function onClose() {
    ...
    navigate({ to: '.', hash: '', resetScroll: false });
    setOpens(false);
  }
  ...
  return (
    <Sheet open={opens} onOpenChange={handleOpenChange}>
      <SheetContent
        side="right"
                          ...
                          >
                            ...
                            <div className="text-center">
                              <p className="text-xs font-medium sm:text-sm">
                                {uploadFile.isDragActive ? 'Lepaskan file di sini...' : 'Unggah Grand Design Tim Anda'}
                              </p>
